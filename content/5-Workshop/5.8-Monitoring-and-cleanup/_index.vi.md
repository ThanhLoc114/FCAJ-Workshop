...
Phần này giải thích cách giám sát backend AWS BILLO và dọn dẹp tài nguyên AWS sau khi hoàn thành workshop.

Việc giám sát rất quan trọng vì backend chạy trên các dịch vụ serverless như API Gateway, Lambda, DynamoDB, S3 và Cognito. Khi xảy ra lỗi, CloudWatch Logs và AWS Console có thể giúp xác định nguyên nhân.

Việc dọn dẹp tài nguyên cũng quan trọng để tránh phát sinh chi phí AWS không cần thiết sau khi kiểm thử.

---

## 1. Giám sát với CloudWatch Logs

AWS BILLO sử dụng Amazon CloudWatch Logs để theo dõi hoạt động backend và gỡ lỗi Lambda/API.
...
## 2. Dọn dẹp tài nguyên AWS

Sau khi hoàn thành workshop, cần xóa các tài nguyên backend đã triển khai nếu không còn nhu cầu sử dụng.

Vì backend được triển khai bằng AWS SAM, có thể dọn dẹp tài nguyên bằng SAM hoặc CloudFormation.
...
## Lưu ý quan trọng khi dọn dẹp tài nguyên
...
Nếu dự án vẫn đang được phát triển, không nên xóa backend stack. Thay vào đó, chỉ nên xóa các dữ liệu kiểm thử không còn cần thiết.
...
## Tùy chọn: Cấu hình cảnh báo ngân sách AWS
...
## Lỗi thường gặp khi dọn dẹp tài nguyên
